...
def get_answer(text, user_key)
    ...
    data_header = {
        ...
        'Authorization' 'Bearer +본인의 dialogflow api+', #ed47255b~cb87 부분에 본인의 dialog flow api key를 입력(Bearer띄어쓰기유의)
        
        'Content-Type' 'applicationjson; charset=utf-8'
    }
    
    dialogflow_url = 'httpsapi.dialogflow.comv1queryv=20150910'   
...
def get_message(bot, update)
    ...
    dialog_answer = get_answer(content, str(chat_id)) #사용자의 메시지에 대한 dialogflow의 답변이 리턴
...
def main()
    updater = Updater('+본인이 생성한 텔레그램 봇 토큰 입력+') #본인이 생성한 텔레그램 봇 토큰 입력
...
def index()
    return 'h1welcome to chatbot worldh'
